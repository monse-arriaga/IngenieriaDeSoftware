    ...
    <div class="q-pa-md">
      ...
      >
        ...
        >
          ...
          <q-input dark filled label-color="white" v-model="nombre" label="Nombre del torneo" :input-style="{ color: 'white' }" />
          
          <div style="margin-top: 20px;"></div>

          <div class="input-container">
            <!-- Input para la fecha del torneo -->
            <q-input dark filled label-color="white" v-model="fecha" label="Fecha del torneo" type="date" :input-style="{ color: 'white'}" style="max-width: 3000px" />
            
            <!-- Espacio entre los inputs -->
            <div style="width: 20px;"></div>
            
            <!-- Input para la hora del torneo -->
            <q-input dark filled label-color="white" v-model="hora" label="Hora del torneo" type="time" :input-style="{ color: 'white'}" style="max-width: 3000px" />
          </div>

        <!-- Selector de opciones múltiples -->
        <div style="margin-top: 40px;"></div>
        <div class="q-gutter-md">
        <q-select dark filled v-model="tipoTorneo" :options="options" label="Selecciona el tipo de torneo" />
        </div>

        <div style="margin-top: 40px;"></div>
        <div class="input-container">
        <q-input dark filled v-model.number="noJugadores" type="number" label="Personas por torneo"  style="max-width: 350px"
        />

        <!-- Espacio entre los inputs -->
        <div style="width: 20px;"></div>

        <q-input dark filled v-model.number="jugadoresXequipo" type="number" label="Personas por equipo"  style="max-width: 350px"/>
        </div>
